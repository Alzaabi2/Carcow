...
def getCarData(make,model,year,zip, stopIdx, pricePriority, mileagePriority, yearPriority, trim, trimPriority):
    cursor = mydb.cursor(dictionary=True)
    ipAddress = request.remote_addr # Get the IP address of the user
    ...
    if pricePriority == mileagePriority == yearPriority == trimPriority:
        ...
        sortedCars = getTopCars(list, rating)
        print("\n4. From the list of the rated cars, the top 5 are retrieved, with all relevant data from the database \n")
        time7 = time.perf_counter()
        # print("Timer6 topCars():" + str(time7-time6))
        ...
        with open('TempData.txt', 'w', encoding='utf8', newline='\n') as f:
            ...
            for i in range(len(sortedCars)):
                if i < stopIdx:
                    row = [1, sortedCars[i]['VIN'], sortedCars[i]['make'], sortedCars[i]['model'], sortedCars[i]['year'], sortedCars[i]['trim'], sortedCars[i]['mileage'], sortedCars[i]['price'], sortedCars[i]['suggested'], sortedCars[i]['url'], sortedCars[i]['imageurl']]
                    w.writerow(row)
                    continue

                row = [0, sortedCars[i]['VIN'], sortedCars[i]['make'], sortedCars[i]['model'], sortedCars[i]['year'], sortedCars[i]['trim'], sortedCars[i]['mileage'], sortedCars[i]['price'], sortedCars[i]['suggested'], sortedCars[i]['url'], sortedCars[i]['imageurl']]
                w.writerow(row)
        ...
        for i in range(len(sortedCars)):
            available = False
            if "https:" not in sortedCars[i]['imageurl']:
                sortedCars[i]['imageurl'] = "https:" + sortedCars[i]['imageurl']
            
            available = availabilityCheck(sortedCars[i]['url'])

            if available:
                topCars.append(sortedCars[i])
                count += 1

            #Break when you reach stopIdx
            if count >= stopIdx - 1:
                break
...
getCarData('Hyundai', 'Palisade','2017','22201',5,0,0,0,'NA',0)
